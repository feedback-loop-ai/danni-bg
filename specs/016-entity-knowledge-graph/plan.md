...
**Testing**: `bun:test` (the project test framework) — repo unit tests, materialiser unit tests, endpoint route tests (full suite green for the PR)  
...
- **VIII. 100% Test Coverage & Endpoint Parity**: PASS. The new pure modules — the predicate vocabulary, `EntityRelationsRepo` (assert/read-both-directions/idempotent), and `registerEntityRelations` (present-only, parent upsert, idempotent) — are unit-tested to the constitution's 100% line+branch coverage gate; the `GET /api/entities/:id` endpoint is covered by an integration test (typed relations both directions + 404). The endpoint's contract test is registered in the parity matrix; here that matrix is reused as the registry for an **internal explorer-API contract test** (distinct from portal-endpoint parity — see Principle IX note and `research.md`). No render-glue exception is invoked here.
