...
- [ ] T001 [P] Confirm the baseline `runIndex` loop and its stores are as plan.md describes: `src/index/run-index.ts` re-embeds **every** active dataset and deletes FTS rows only for non-active datasets it visits (never touches `dataset_embeddings`); `src/index/embeddings-store.ts` exposes `deleteEmbedding`/`ensureEmbeddingsTable`/`getEmbeddingsMeta`/`setEmbeddingsMeta`; `src/index/vec.ts` exposes `composeEmbeddingText`; `src/index/fts.ts` exposes `buildFtsRow`/`upsertFtsRow`/`deleteFtsRow` and the `FtsRow` interface. Run `bun test tests/unit/index/` and `bun run lint` to confirm a green starting point. (No code change; this is the pre-flight that anchors every file path cited below.)
...
- [ ] T012 [US1] Integration test SC-002 (changed-only re-embed) in `tests/integration/index-incremental.test.ts`: mutate exactly one dataset's embedding input (title/description/translation/entity), re-run → `ftsUpdated:1`, `vectorsUpdated:1`, `embedded:1`, `skippedUnchanged:N-1`; a newly-added dataset is embedded without re-embedding any unchanged one (US1 scenarios 2–3).
...
- [ ] T026 [US2] In `src/index/run-index.ts`, classify a vector recompute caused by `state.model_id !== currentModelId` (with `embed_fp` matching) as `reembeddedDueToModelChange` rather than `embedded`; a content-driven re-embed (changed/NULL `embed_fp`) stays `embedded`; ensure the FTS leg is never triggered by a model-only change (FR-004 — model change re-embeds vectors only). On every vector persist, `indexState.upsertEmbed` writes the new `embed_fp` + `currentModelId` (T021 already does the write; this task is the counting/branch logic). Satisfies the T022 model-change unit tests. (Depends on T021, T025.)
