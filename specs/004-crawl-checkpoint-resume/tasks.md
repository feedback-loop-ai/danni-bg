...
Not started. The plan's Phase 0 (research.md) is complete; all design unknowns are resolved (research.md R1–R9). This task list realizes plan.md Phases 1–5.
...
- [ ] T236 [P] Validate `specs/004-crawl-checkpoint-resume/quickstart.md` end-to-end against the implemented CLI in `tests/integration/quickstart-004.test.ts`: every path-like reference resolves, the documented `sqlite3` checkpoint queries (§1, §2, §5) match the shipped schema, and the §7 mutual-exclusion exit-code-5 example holds.
- [ ] T237 [P] Update `danni sync --help` in `src/cli/sync.ts` to document `--retry-failed` and clarify that `--max` is the per-session **dataset** batch that advances and persists the cursor (quickstart.md §1, §6; research.md R6).
- [ ] T238 [P] Confirm parity matrix is unaffected (`tests/parity-matrix.json`): this feature consumes no NEW portal endpoint (reuses `EgovBgClient.listDatasets`/`getDatasetDetails`/`listResources`/`getResourceData`/`listOrganisations` already covered); add a note in `tests/integration/constitution-gates.test.ts` if a new egov method is introduced (it is not).
- [ ] T239 Final coverage audit: `bun test --coverage` reports 100% line + branch over every authored module for this feature — `src/crawler/{scope-hash,egov-validator,crawl-checkpoint,run-egov-sync}.ts`, the refactored `src/crawler/egov-sync.ts`, `src/store/repos/crawl-checkpoints.ts`, and the `src/cli/{sync,status}.ts` deltas — and the unit suite stays within the <5s budget (Constitution VI, VIII). The per-resource checkpoint commit is a single small `UPDATE` (plan.md Performance Goals).
