...
import { isNumeric, numericColumns, toSeries } from './chart.ts';
...
describe('toSeries', () => {
  ...
  it('skips rows whose value is non-numeric', () => {
    expect(toSeries([{ v: 'x' }, { v: 5 }], null, 'v').points).toEqual([{ label: '2', value: 5 }]);
  });
});
